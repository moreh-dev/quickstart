...
try:
    ScaledDotProductionAttetion = torch.ops.moreh.scaled_dot_product_attention
except:
    assert False
    ScaledDotProductionAttetion = None
...
class BaichuanAttention(torch.nn.Module):
    ...
    def forward(
        ...
        proj = proj.split(self.hidden_size, dim=2)
        ...
        elif ScaledDotProductionAttetion is not None and not output_attentions and self.training:
            attn_weights = None
            query_states = query_states.transpose(1, 2).reshape(bsz, q_len, self.hidden_size)
            key_states = key_states.transpose(1, 2).reshape(bsz, q_len, self.hidden_size)
            value_states = value_states.transpose(1, 2).reshape(bsz, q_len, self.hidden_size)
            attention_mask = torch.max(attention_mask, torch.tensor(torch.finfo(attention_mask.dtype).min))
            scale_factor = 1.0 / float(self.head_dim)**0.5

            attn_output = ScaledDotProductionAttetion(
                    query_states,
                    key_states,
                    value_states,
                    attention_mask,
