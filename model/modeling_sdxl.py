...
class SDXL(nn.Module):
    ...
    def forward(self, images, tokens=None, snr_gamma=5):
        ...
        if not self.ignore_vae:
            latents = self._encode_image(VaeImageProcessor.normalize(images)).to(
                self.dtype
            )
        else:
            b, _, h, w = images.shape
            latents = torch.rand(
                (b, 4, h // VAE_DOWNSCALE_FACTOR, w // VAE_DOWNSCALE_FACTOR),
                dtype=self.dtype,
            ).cuda()
        ...
        add_time_ids = self._get_add_time_ids(
            (h * VAE_DOWNSCALE_FACTOR, w * VAE_DOWNSCALE_FACTOR)
        )
        ...
        mse_loss_weights = (
            torch.stack([snr, self.snr_gamma * torch.ones_like(timesteps)], dim=1).min(
                dim=1
            )[0]
            / snr
        )
        ...
        if is_lora:
            ...
            if train_text_encoder:
                text_encoder_lora_layers = convert_state_dict_to_diffusers(
                    get_peft_model_state_dict(self.text_encoder)
                )
