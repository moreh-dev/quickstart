...
class MistralFlashAttention2(MistralAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if MorehFlashAttention is None:
        ...
        if not _flash_supports_window_size and MorehFlashAttention is None:
            logger.warning_once(
        ...
        if MorehFlashAttention is None:
            ...
            attn_output = MorehFlashAttention(
                query_states,
                key_states,
                value_states,
                attention_mask,
...
class MistralSdpaAttention(MistralAttention):
    ...
    def forward(
        ...
        if ScaledDotProductAttention is not None and not output_attentions and self.training:
            query_states = query_states.transpose(1, 2).reshape(
                bsz, q_len, self.hidden_size)
            key_states = key_states.transpose(1, 2).reshape(
                bsz, q_len, self.hidden_size // self.num_key_value_groups)
            value_states = value_states.transpose(1, 2).reshape(
                bsz, q_len, self.hidden_size // self.num_key_value_groups)

            batch, query_length, key_length = query_states.size(
                0), query_states.size(-2), key_states.size(-2)
            masked_bias = attention_mask.expand(batch, self.num_heads,
            ...
            attn_output = ScaledDotProductAttention(
                query_states,
                key_states,
                value_states,
                masked_bias,
...
MISTRAL_ATTENTION_CLASSES = {
    "eager": MistralFlashAttention2,
    "flash_attention_2": MistralFlashAttention2,
    "sdpa": MistralFlashAttention2,
}
...
class MistralDecoderLayer(nn.Module):
    def __init__(self, config: MistralConfig, layer_idx: int):
        ...
        RMSNorm = MorehRMSNorm if MorehRMSNorm is not None else MistralRMSNorm
