    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if not self.warmup_checker:
            ...
            if state.is_local_process_zero:
                logger.info(f"[Step {state.global_step}] Model loading duration : {self.warmup_duration:.2f} sec")
            self.start = time.time()
            self.accum = 0
            ...
            if 'loss' in logs:
                ...
                if state.is_local_process_zero:
                    logger.info(f"[Step {state.global_step}] | TPS: {tps:.2f} tokens/sec | Loss: {loss:.4f} | LR : {lr:.8f} | Duration for 1 Step: {duration / self.accum:.2f} sec")
            self.accum = 0
            self.start = time.time()
